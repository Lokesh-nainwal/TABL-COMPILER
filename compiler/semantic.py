...
DATABASE = {}  # In-memory database with schema and rows
...
def check_ast(ast):
    ...
    if command_type == "CREATE":
        ...
        for col in schema:
            ...
            # Type enforcement with error clarity
            try:
                if col_type == "int":
                    val = int(val)
        ...
        group_by = ast.get("group_by")  # ✅ GROUP BY ADDED
        ...
        #             grouped[key] = row  # Just return one representative row per group
        #     return list(grouped.values())
        # Instead of returning the whole row, just return the requested columns
        if group_by:
            grouped = {}
            for row in filtered:
                key = row.get(group_by)
        ...
        return f"🗑️ Deleted {deleted_count} row(s) from '{table_name}'."
